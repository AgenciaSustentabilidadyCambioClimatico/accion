class Flujo < ApplicationRecord
  ...
  def instancias_del_flujo_fpl current_user
    ...
    # DEDUPLICACIÓN DINÁMICA MEDIANTE CLAVES ÚNICAS (TAREA + RENDICIÓN)
    tareas_procesadas_keys = Set.new
    revision = 1
    revision_juridica = 1
    ...
    self.tarea_pendientes.order(id: :asc).each do |tarea_pend|
      ...
      codigo = tarea_pend.tarea.codigo.to_s

      # DEDUPLICAR SI LA TAREA ES GENERADA PARA MÚLTIPLES ACTORES EN PARALELO
      if ['FPL-02', 'FPL-13', 'FPL-16'].include?(codigo)
        d_hash = if tarea_pend.respond_to?(:data_parsed)
                   tarea_pend.data_parsed
                 elsif tarea_pend.data.is_a?(Hash)
                   tarea_pend.data
                 elsif tarea_pend.data.is_a?(String) && tarea_pend.data.present?
                   (YAML.safe_load(tarea_pend.data, permitted_classes: [Symbol, Date, Time, ActiveSupport::HashWithIndifferentAccess]) rescue {})
                 else
                   {}
                 end
        d_hash = d_hash.with_indifferent_access if d_hash.respond_to?(:with_indifferent_access)

        rend_id = d_hash[:rendicion_fpl_id].presence || (tarea_pend.determina_rendicion.id rescue nil)
        mes_rend = d_hash[:mes_a_rendir].presence || (tarea_pend.determina_rendicion.mes_a_rendir rescue nil)

        clave_unica = if rend_id.present?
                        "#{codigo}_rend_#{rend_id}"
                      elsif mes_rend.present?
                        "#{codigo}_mes_#{mes_rend}"
                      else
                        "#{codigo}_lote_#{tarea_pend.created_at.to_i / 30}"
                      end

        next if tareas_procesadas_keys.include?(clave_unica)
        tareas_procesadas_keys.add(clave_unica)
      end
      ...
      if tarea_pend.tarea.es_convocatoria?
        ...
        tps = self.tarea_pendientes.where(tarea_id: tarea_pend.tarea.id)
        convocatorias_ids = []
        tps.each do |tp|
          convocatorias_ids << tp.data[:convocatoria_id] if (tp.data.present? && tp.data.has_key?(:convocatoria_id))
        end
      ...
      tareas_validaciones_fpl_05 = []
      if tarea_pend.tarea.codigo == Tarea::COD_FPL_05
        tarea_fondo_fpl_09 = Tarea.find_by_codigo(Tarea::COD_FPL_09)
        existe_fpl_09 = TareaPendiente.where(tarea_id: tarea_fondo_fpl_09.id, flujo_id: self.id).count
        ...
        maximo = TareaPendiente.where(tarea_id: tarea_fondo.id, flujo_id: self.id).count
        if existe_fpl_09 >= 1
          if revision_juridica <= maximo && revision_juridica <= existe_fpl_09
            tareas_validaciones_fpl_05 << revision_juridica
            documentos_asociados = [{nombre: "", url: "", parametros: [], metodo: false}]
            revision_juridica += 1
          end
        end
