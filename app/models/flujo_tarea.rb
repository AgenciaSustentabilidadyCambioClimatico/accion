class FlujoTarea < ApplicationRecord
	...
	def continuar_flujo flujo_id, extra = {}, sin_destinatario = false
		...
		unless self.sin_salida
			...
			if sig_tarea.codigo == Tarea::COD_FPL_00
				tarea_fondo = Tarea.find_by_codigo(Tarea::COD_APL_001)
                postulante = TareaPendiente.find_by(tarea_id: tarea_fondo.id, flujo_id: flujo_id)
				fondo = FondoProduccionLimpia.where(flujo_apl_id: flujo_id).first
          		tp = TareaPendiente.find_or_create_by({
					flujo_id: fondo.flujo_id,
				...
				if ((!self.mensaje_salida_asunto.blank?) && (!self.mensaje_salida_cuerpo.blank?))
					rgc = RegistroAperturaCorreo.create(user_id: postulante.user_id, flujo_tarea_id: tarea_fondo.id, fecha_envio_correo: DateTime.now, flujo_id: fondo.flujo_id)
					begin
						fpl = fondo
					...
					FlujoMailer.enviar(
						self.asunto_format(postulante.user, mdi, fpl), 
						self.cuerpo_format(postulante.user, mdi, fpl), 
							...
							unless actor.nil?
								...
								(1..repeticiones).each do |r|
									...
									unless ((sig_tarea.codigo == Tarea::COD_APL_032) && auditorias.blank?)
										...
										if ((!self.mensaje_salida_asunto.blank?) && (!self.mensaje_salida_cuerpo.blank?) && (self.tarea_entrada.codigo != Tarea::COD_FPL_006) && !(sig_tarea.codigo == Tarea::COD_APL_032 && tp.present? && !tp.new_record?))
