class EncuestaUserRespuesta < ApplicationRecord
	...
	belongs_to :user
	belongs_to :tarea_pendiente, optional: true
	...
	def cerrar(user_id,flujo_id, institucion_proveedor_id, tarea_pendiente_id) #DZC se agrega el flujo_id y la institución del responsable de entregables
		creadas = false
		unless self.tiene_errores?
			...
			self.preguntas_y_respuestas.each do |pid,respuesta|
				...
				respuestas << { encuesta_id: self.encuesta_id, user_id: user_id, pregunta_id: pid, respuesta: respuesta[respuesta.keys.first], flujo_id: flujo_id, institucion_proveedor_id: institucion_id, tarea_pendiente_id: tarea_pendiente_id}
			end
