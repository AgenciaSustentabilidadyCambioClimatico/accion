class MinutasController < ApplicationController #crea la depencia con convocatoria en la carpeta del controlado
	...
	before_action :set_convocatoria, except: [:archivo]
	before_action :set_tipo, except: [:archivo]
	before_action :set_descargable_tareas, except: [:archivo]
	before_action :set_minuta, only: [:edit, :update, :destroy], except: [:archivo]
	before_action :permiso_tarea, except: [:archivo]
	respond_to :docx
	# GET /minuta/new
	def new
...
#DZC agrega al campo data de la tarea_pendiente 
	...
	def archivo		
		...
		respond_to do |format|
	    format.docx do
	      render docx: 'archivo', filename: 'my_file.docx'
	    end
	  end
	end
