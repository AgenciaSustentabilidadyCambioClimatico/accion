class ActoresConvocatoriasController < ApplicationController
				...
				if @convocatoria.save
		  		...
		  		#crea listado de convocados como destinatarios del correo
					...
					@convocatoria.convocatoria_destinatarios.each do |rd|
						...
						ConvocatoriaMailer.enviar(rd, @convocatoria.mensaje_encabezado, @convocatoria.mensaje_cuerpo, @convocatoria.archivo_adjunto, rgc.id).deliver_now # DZC 2018-10-11 12:07:48 se corrige error en nombre de variable rgc.id
					end
  		...
  		format.js {
				if @convocatoria.update(convocatoria_params) #DZC no es necesario el merge por que el value preexiste desde el create, pero se deja como ilustración
		  		...
		  		#crea listado de convocados como destinatarios del correo
					...
					@convocatoria.convocatoria_destinatarios.each do |rd|
						rgc = RegistroAperturaCorreo.create(convocatoria_destinatario_id: rd.id, fecha_envio_correo: DateTime.now)
					ConvocatoriaMailer.enviar(rd, @convocatoria.mensaje_encabezado, @convocatoria.mensaje_cuerpo, @convocatoria.archivo_adjunto, rgc.id).deliver_now # DZC 2018-10-11 12:06:07 se corrige error en nombre de variable rgc.id
					end
