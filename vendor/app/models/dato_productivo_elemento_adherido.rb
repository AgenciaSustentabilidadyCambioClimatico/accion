	...
	def self.generar_data (datos_productivos)
		...
		unless datos_productivos.blank?
			datos_productivos.each do |val|
				...
				(1..48 - datos_levantados.count).each do
					data << {rut_institucion: elemento_adherido.fila[:rut_institucion].to_s, #0
						nombre_institucion: elemento_adherido.fila[:nombre_institucion].to_s, #1
						alcance: elemento_adherido.fila[:alcance].to_s, #2
						identificador_elemento_adherido: elemento_adherido.id, #3
	...
	def self.data_dato_productivo_elementoAdherido(excell, archivos)
		...
		if data.size <= 0
			...
			data.each_with_index do |dato, fila|
				vacio = dato[:id_dato_levantado_mensual].blank? && dato[:nombre_archivo_evidencia].blank? && dato[:fecha_levantamiento].blank? && dato[:rut_levantador_datos_terreno].blank? && dato[:unidad_declarada].blank? && dato[:mes].blank? && dato[:año].blank? && dato[:tipo_valor].blank? && dato[:valor].blank?
				...
				if vacio
					...
					if dato_levantado.valid?
					  datos_levantados << dato_levantado
						...
						unless dato_levantado.errors[:fecha_invalida].present? || dato_levantado.errors[:unidades_compatibles].present? || dato_levantado.errors[:archivo_invalido].present? || archivos_referenciados.nil? || dato_levantado.nombre_archivo_evidencia.nil?
