deploy=YAML.load_file(ERB.new(File.read("#{Dir.pwd}/config/deploy.yml")).result)["production"]
  ...
  if ENV['only_backup'].nil? || ENV['only_backup'] == false
    on ( server deploy["server"]["host"], user: deploy["server"]["user"], roles: %w{app web db} ) do |host|
      execute("rm -rf #{deploy['path']}/tables")
    	...
    	execute("psql -h localhost -U postgres -d #{deploy["application"]}_prod -a -f #{deploy['path']}/tables/db_update.sql")
    	execute("for backup in #{deploy['path']}/tables/*.backup; do pg_restore -h localhost -U postgres -d #{deploy["application"]}_prod -1 -a $backup; done")
  	end
  end
end
